...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post('/api/change-password', isAuthenticated, async (req: any, res) => {
    try {
      ...
      // Check bcrypt hash
      if (user.hashedPassword.startsWith('$2b$')) {
        isCurrentValid = await bcrypt.compare(currentPassword, user.hashedPassword);
      } else {
        // Legacy scrypt hash
        const [hashed, salt] = user.hashedPassword.split(".");
        if (salt) {
          const hashedBuf = Buffer.from(hashed, "hex");
          const suppliedBuf = (await scryptAsync(currentPassword, salt, 64)) as Buffer;
          isCurrentValid = timingSafeEqual(hashedBuf, suppliedBuf);
        }
      }
